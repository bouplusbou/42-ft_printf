...
unsigned long long int get_arg_oux(t_struct data, va_list list)
{
...
char *small_res_oux(t_struct data, unsigned long long int arg)
{
	char *small_res;
	int small_res_len;
	char *arg_str;
...
char *create_res_oux(t_struct data, int result_len, char *small_res)
{
	char *result;
	int small_res_len;
	int sign_i;
...
/*
** convert into oux, do all the transformations,
...
int conv_oux(t_struct *data, va_list list)
{
	...
	if (!(small_res = small_res_oux(*data, arg))) // Converti le valeur en brut (Sans prendre en compte la size (petit resultat))
		return (0);
